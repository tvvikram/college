      ...
      if (ConstantSDNode *N1C = dyn_cast<ConstantSDNode>(N1.getNode()))
        {
          ...
          switch (Cond)
            ...
            case ISD::SETEQ:
              return getConstant(C1 == C2, VT);
            case ISD::SETNE:
              return getConstant(C1 != C2, VT);
            case ISD::SETULT:
              return getConstant(C1.ult(C2), VT);
            case ISD::SETUGT:
              return getConstant(C1.ugt(C2), VT);
            case ISD::SETULE:
              return getConstant(C1.ule(C2), VT);
            case ISD::SETUGE:
              return getConstant(C1.uge(C2), VT);
            case ISD::SETLT:
              return getConstant(C1.slt(C2), VT);
            case ISD::SETGT:
              return getConstant(C1.sgt(C2), VT);
            case ISD::SETLE:
              return getConstant(C1.sle(C2), VT);
            case ISD::SETGE:
              return getConstant(C1.sge(C2), VT);
            }
        }
    }
      ...
      if (ConstantFPSDNode *N2C = dyn_cast<ConstantFPSDNode>(N2.getNode()))
        {
          ...
          switch (Cond)
            ...
            case ISD::SETEQ:
              ...
              // fall through
            case ISD::SETOEQ:
              return getConstant(R==APFloat::cmpEqual, VT);
            case ISD::SETNE:
              if (R==APFloat::cmpUnordered)
                return getUNDEF(VT);
              // fall through
            case ISD::SETONE:
              return getConstant(R==APFloat::cmpGreaterThan ||
            ...
            case ISD::SETLT:
              ...
              // fall through
            case ISD::SETOLT:
              return getConstant(R==APFloat::cmpLessThan, VT);
            case ISD::SETGT:
              if (R==APFloat::cmpUnordered)
                return getUNDEF(VT);
              // fall through
            case ISD::SETOGT:
              return getConstant(R==APFloat::cmpGreaterThan, VT);
            case ISD::SETLE:
              if (R==APFloat::cmpUnordered)
                return getUNDEF(VT);
              // fall through
            case ISD::SETOLE:
              return getConstant(R==APFloat::cmpLessThan ||
            ...
            case ISD::SETOGE:
              return getConstant(R==APFloat::cmpGreaterThan ||
                                 R==APFloat::cmpEqual, VT);
            case ISD::SETO:
              return getConstant(R!=APFloat::cmpUnordered, VT);
            case ISD::SETUO:
              return getConstant(R==APFloat::cmpUnordered, VT);
            case ISD::SETUEQ:
    ...
    ConstantSDNode *Cst2)
{
  ...
  switch (Opcode)
    ...
    case ISD::SHL:
      return getConstant(C1 << C2, VT);
    case ISD::SRL:
      return getConstant(C1.lshr(C2), VT);
    case ISD::SRA:
      return getConstant(C1.ashr(C2), VT);
    case ISD::ROTL:
      return getConstant(C1.rotl(C2), VT);
    case ISD::ROTR:
      return getConstant(C1.rotr(C2), VT);
    default:
      break;
    }
...
SDVTList SelectionDAG::getVTList(const EVT *VTs, unsigned NumVTs)
{
  switch (NumVTs)
    {
    case 0:
      llvm_unreachable("Cannot have nodes without results!");
    case 1:
      return getVTList(VTs[0]);
    case 2:
      return getVTList(VTs[0], VTs[1]);
    case 3:
      return getVTList(VTs[0], VTs[1], VTs[2]);
    case 4:
      return getVTList(VTs[0], VTs[1], VTs[2], VTs[3]);
    default:
      break;
    }
    ...
    DAGUpdateListener *UpdateListener)
{
  ...
  // mycode here
/// MakePureDAG - Make the uses of FrameIndex, the uses of the corresponding
/// load/store instructions.
void SelectionDAG::MakePureDAG() {
  SDNode *entry = getEntryNode().getNode(), *LoadStore, *FrameIndex, *Load;
  for (SDNode::use_iterator I = entry->use_begin(), E = entry->use_end();
        I != E; ++I) {
    SDNode *x = *I;
    if (x->getNodeType() == ISD::LOAD || x->getNodeType() == ISD::STORE)
      Load = *I;
  }
  for (SDNode::use_iterator I = Load->use_begin(), E = Load->use_end();
        I != E; ++I)  {
    SDNode *temp = *I;
    if (temp->getNodeType() == ISD::LOAD || temp->getNodeType() == ISD::STORE) {
      LoadStore = temp;
      for (unsigned i = 0; i < LoadStore->getNumOperands(); i++) {
        SDNode *temp = LoadStore->getOperand(i).getNode();
  ...
  FrameIndex->dump();
  int i = 0;
  for (SDNode::use_iterator I = FrameIndex->use_begin(),
        E = FrameIndex->use_end(); I != E; ++I) {
    SDNode *Uses = *I;
    Uses->dump();
    i++;
  }
  // Now add the uses of FrameIndex to the uses of LoadStore
  int j = 0;
  for (SDNode::use_iterator I = FrameIndex->use_begin(),
        E = FrameIndex->use_end(); I != E && j <= i; ++I) {
    SDNode *Uses = *I;
    if (Uses != LoadStore) {
    // SDUse U = I.getUse();
      Uses->dump();
      LoadStore->addUse(I.getUse());
      j++;
    }
  }
}

/// InitializeNodes - This will initialize all nodes as not in any lineage
/// but sets some nodes like	FIXME

/*
/// LowestDescendant - Returns the lowest descendant of a node
SDNode *SelectionDAG::LowestDescendant(SDNode *N) {
  int h = 0;
  SDNode *ld;
  for (SDNode::use_iterator I = N->use_begin(), E = N->use_end(); I != E; ++I) {
    SDNode *temp = *I;
    if (I.getUse().getValueType() == MVT::Other)
      DEBUG(errs() << "Blue edged node\n");
    else if (I.getUse().getValueType() == MVT::Flag)
      DEBUG(errs() << "Red edged node\n");
    else {
...
/// in any lineage
SDNode *SelectionDAG::getHighestNodeNotInAnyLineage() {
...
/// MarkAllNodesAsNotInAnyLineage - Make the IsInLineage field of all
/// nodes as false. Returns the total number of nodes traversed.
unsigned SelectionDAG::MarkAllNodesAsNotInAnyLineage() {
  unsigned nodesTraversed = 0;
  for (allnodes_iterator I = allnodes_begin(), E = allnodes_end(); I != E;
        nodesTraversed++) {
    SDNode *N = I++;
    N->setIsInLineage(false);
    // N->dump();std::cout << "Lineage: " << N->getIsInLineage() << "\n";
  }
  return nodesTraversed;
}

/// PrintAllNodes - Print all the nodes of the DAG
void SelectionDAG::PrintAllNodes() {
  for (allnodes_iterator I = allnodes_begin(),E = allnodes_end(); I != E; ++I) {
    SDNode *temp = I;
    temp->dump();
    std::cout << "Height: " << temp->getHeight() << "\n";
  }
}

/// PrintSequenceNodes - Print the nodes connected by sequencing edges
void SelectionDAG::PrintSequenceNodes() {
  std::cout << "Sequencing Edged nodes:\n";
  for (allnodes_iterator I = allnodes_begin(),E = allnodes_end(); I != E; ++I) {
    SDNode *N = I;
    if (SDNode *SE = N->getSequencingEdge()) {
      N->dump();
      SE->dump();
    }
  }
...
/// setVisitedHeightToFalse - Set VisitedHeight of all nodes to false
void SelectionDAG::setVisitedHeightToFalse() {
  for (allnodes_iterator I = allnodes_begin(),E = allnodes_end(); I != E; ++I) {
    SDNode *N = I;
    N->setVisitedHeight(false);
  }
}

/// setIsScheduledToFalse - Set IsScheduled of all nodes to false
void SelectionDAG::setIsScheduledToFalse() {
  for (allnodes_iterator I = allnodes_begin(),E = allnodes_end(); I != E; ++I) {
    SDNode *N = I;
    N->setIsScheduled(false);
    if (N->getNodeType() == ISD::UNDEF)
      N->setIsScheduled(true);
  }
}

/// CheckForPredsSched - Return true if all the predecessors of SDNode are
/// scheduled.
bool SelectionDAG::CheckForPredsSched(SDNode *N) {
  setVisitedHeightToFalse();
  for (unsigned i = 0, e = N->getNumOperands(); i != e; ++i) {
    SDNode *Op = N->getOperand(i).getNode();
    // Check if N is true dependant on Op, that is there is no blue/red edge
    // between Op and N
    for (SDNode::use_iterator I = Op->use_begin(), E = Op->use_end();
          !Op->getVisitedHeight() && I != E; ++I) {
      SDNode *temp = *I;
      if (I.getUse().getValueType() == MVT::Other)
        DEBUG(errs() << "CPredSched: Blue edged node\n");
      else if (I.getUse().getValueType() == MVT::Flag)
        DEBUG(errs() << "CPredSched: Red edged node\n");
      else if (temp == N)
        Op->setVisitedHeight(true);
    }
    if (Op->getVisitedHeight() && !(SDNode::isLeafNode(Op->getNodeType()))
          && !(Op->getIsScheduled()))
      return false;
  }
  // If there is a sequencing edge to N, then N has a predecessor
  for (allnodes_iterator I = allnodes_begin(),E = allnodes_end(); I != E; ++I) {
    SDNode *Op = I;
    if (Op->getSequencingEdge() == N && !(Op->getIsScheduled()))
      return false;
  }
  return true;
}

/// PrintHeightOfNodes - Print the height of all nodes in the DAG.
/// Also print the NumDescendants.
void SelectionDAG::PrintHeightOfNodes() {
  for (allnodes_iterator I = allnodes_begin(),E = allnodes_end(); I != E; ++I) {
    SDNode *N = I;
    N->dump();
    std::cout << N->getHeight() << " " << N->getNumDescendants() << "   ";
  }
}

/// ComputeHeightOfEveryNode - Compute the height of every node in the ddg.
/// MRIS
void SelectionDAG::ComputeHeightOfEveryNode() {
  // Set VisitedHeight of every node to false and Height to -1
  for (allnodes_iterator I = allnodes_begin(),E = allnodes_end(); I != E; ++I) {
    SDNode *N = I;
    N->setVisitedHeight(false);
    N->setHeight(-1);
    N->setNumDescendants(0);
    N->clearDescendants();
  }
  // Compute the height of every node, send EntryNode as the root
  SDNode *entry = getEntryNode().getNode();
  int height, max = 0;
  for (SDNode::use_iterator I = entry->use_begin(), E = entry->use_end();
        I != E; ++I) {
    SDNode *temp = *I;
    if (temp->getVisitedHeight() == false) {
      DEBUG (errs() << temp->dump());
      temp->setVisitedHeight(true);
      entry->setNumDescendants(entry->getNumDescendants() + 1);
      entry->setDescendant(temp);
      ComputeHeightOfNode(temp);
    }
  }
  entry->setHeight(-1);
  // Now check if there are any nodes left whose height has not yet calculated
  DEBUG (errs() << "Computing height for leftout nodes:\n");
  height = 0, max = 0;
  for (allnodes_iterator I = allnodes_begin(),E = allnodes_end(); I != E; ++I) {
    SDNode *temp = I;
    if (temp->getVisitedHeight() == false && (temp->getNodeType() == ISD::LOAD
          || temp->getNodeType() == ISD::STORE)) {
      // temp->dump();
      temp->setVisitedHeight(true);
      temp->setHeight(ComputeHeightOfNode(temp));
    }
  }
  //PrintHeightOfNodes();
}

/// ComputeHeightOfNode - Compute the height of node N by recursively traversing
/// as height (u) = 1, if 'u' has no descendants
///               = 1 + max (descendants height) of 'u', otherwise
int SelectionDAG::ComputeHeightOfNode(SDNode *N) {
  // std::cout << "Inside height compute:\n";
  SDVTList l = N->getVTList();
  unsigned i = 0;
  DEBUG (errs() << "\nl.NumVTs: " << l.NumVTs << "NumValues: "
          << N->getNumValues() << " ");
  /*
  std::cout << "Actual use iteration:\n";
  for (SDNode::use_iterator I = N->use_begin(), E = N->use_end(); I != E; ++I) {
    SDNode *cn = *I;
    cn->dump();
  }
  */
  ...
  int height = 0, max = 0;
  bool leafNode = true;	// Is N a leafNode? if yes, then return height as 1
  for (SDNode::use_iterator I = N->use_begin(), E = N->use_end(); I != E; i++) {
    SDNode *curNode = *I;
    if (I.getUse().getValueType() == MVT::Other 
          /* || curNode->getNodeType() == ISD::TokenFactor*/) {
      DEBUG (errs() << "Blue edged node\n");
      ++I;
    }
    else if (I.getUse().getValueType() == MVT::Flag) {
      ...
      leafNode = false;
      N->setNumDescendants(N->getNumDescendants() + 1);
      N->setDescendant(curNode);
      if (curNode->getVisitedHeight() == false) {
        DEBUG (errs() << "Normal node\n");
        DEBUG (errs() << curNode->dump());
        curNode->setVisitedHeight(true);
        height = ComputeHeightOfNode(curNode);
        if (height > max)
          max = height;
      }
      else {
        height = curNode->getHeight();
        if (height > max)
          max = height;
      }
      ++I;
  ...
  if (leafNode)
    ...
    N->setHeight(max + 1);
  // If N is load/store then move to its FrameIndex and call compute height on
  // it but set the computed height to N and set Height of FrameIndex to -1 this
  // is because, getHighestNodeNotInAnyLineage() should return correct value
  if (N->getNodeType() == ISD::LOAD || N->getNodeType() == ISD::STORE) {
    for (unsigned j = 0; j < N->getNumOperands(); j++) {
      SDNode *temp = (N->getOperand(j)).getNode();
      // If FrameIndex node is found then call compute height on each of its
      // uses except N, otherwise it results in recursion
      if (SDNode::isLeafNode(temp->getNodeType())
            && temp->getVisitedHeight() == false) {
        int cHeight = 0, cMax = 0;
        SDVTList t = temp->getVTList();
        unsigned k = 0;
        temp->setVisitedHeight(true);
        ...
        for (SDNode::use_iterator X = temp->use_begin(), Y = temp->use_end();
          ...
          if (X.getUse().getValueType() == MVT::Other) {
            DEBUG (errs() << "FI: Blue edged node\n");
            ++X;
          }
          else if (X.getUse().getValueType() == MVT::Flag) {
            DEBUG (errs() << "FI: Red edged node\n");
            ++X;
          }
          else {
            SDNode *FIUses = *X;
            if (FIUses != N) {
              if (FIUses->getVisitedHeight() == false) {
                N->setNumDescendants(N->getNumDescendants() + 1);
                N->setDescendant(FIUses);
                DEBUG (errs() << FIUses->dump());
                FIUses->setVisitedHeight(true);
                cHeight = ComputeHeightOfNode(FIUses);
                if (cHeight > cMax)
                  cMax = cHeight;
              }
              else {
                cHeight = FIUses->getHeight();
                if (cHeight > cMax)
                  cMax = cHeight;
              }
            }
  ...
  // Check for sequencing edges, not necessary for a FrameIndex node as there
  // would be no sequence edged node from a FrameIndex node
  SDNode *SequenceNode = N->getSequencingEdge();
  if (SequenceNode != NULL) {
    int height;
    // If sequence edge node not yet visited, then compute height
    // else get the current height and check with the existing height of N
    if (SequenceNode->getVisitedHeight() == false) {
      height = ComputeHeightOfNode(SequenceNode);
      SequenceNode->setVisitedHeight(true);
    }
    else
      height = SequenceNode->getHeight();
    // set height if necessary
    if ((height + 1) > N->getHeight())
  ...
  switch (getOpcode())
    ...
    case ISD::CONVERT_RNDSAT:
    {
      switch (cast<CvtRndSatSDNode>(this)->getCvtCode())
        {
        default:
          llvm_unreachable("Unknown cvt code!");
        case ISD::CVT_FF:
          return "cvt_ff";
        case ISD::CVT_FS:
          return "cvt_fs";
