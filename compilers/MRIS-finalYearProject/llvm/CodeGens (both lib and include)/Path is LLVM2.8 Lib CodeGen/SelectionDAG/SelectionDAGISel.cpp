...
void SelectionDAGISel::CodeGenAndEmitDAG() {
  ...
  // mycode here
  //std::cout << "Initial selection DAG:" << std::endl;
  //CurDAG->dump();
  //CurDAG->viewGraph();
  // mycode ends here
  ...
  // mycode here
  {
    // Instruction sequencing is done here, which is before instruction selection
    // Currently: do sequencing, then decide instruction selection and
    // scheduling. Placement of the current sequencing is not yet final.
    // MRIS Sequencing: Paper: {R.Govindarajan et al.}
    //                  Minimum Register Instruction Sequencing to Reduce
    //                  Register Spills in Out-of-Order Issue Superscalar
    //                  Architectures.
    // Shree RAM :)
    // MRISLineageFormation();
  }
  // mycode ends here
  ...
  // mycode here
/*
void SelectionDAGISel::MRISLineageFormation() {
  DEBUG(errs() << "===== MRIS Lineage Formation begins:\n");
  // A data dependency graph (DDG) is same as a DAG.
  // Get the total number of nodes in the DDG.
  DAGSize = CurDAG->AssignTopologicalOrder();
  DEBUG (errs() << "DAGSize: " << DAGSize << "\n");
  
  // Mark all nodes in the DDG as not in any lineage
  unsigned eqDAGSize = CurDAG->MarkAllNodesAsNotInAnyLineage();
  
  if (eqDAGSize == DAGSize)
    DEBUG (errs()<< "All dags traversed! :); eqDAGSize: " << eqDAGSize << "\n");
  else
    std::cout << "All dags could not be traversed! :(; eqDAGSize: "
              << eqDAGSize << " DAGSize: " << DAGSize << "\n";
  
  // compute the height of every node of CurDAG
  CurDAG->ComputeHeightOfEveryNode();
  CurDAG->PrintAllNodes();

  SDNode *Vi, *Vj;
  SmallVector<Lineage*, 4> LineageList;
  while ((Vi = CurDAG->getHighestNodeNotInAnyLineage()) != NULL) {
    bool RecomputeHeight = false;
    // create a lineage and add it to the LineageList
    Lineage *l;		
    LineageList.push_back(l);
    // Start a new lineage containing Vi by adding it to the LineageList
    // Also mark Vi as in lineage
    l->AddNodeToLineage(Vi);
    Vi->setIsInLineage(true);
    // CurDAG->LowestDescendant(Vi);
    while (Vj != NULL) {
      // if Vi has multiple descendants
      if (Vi->use_size() > 1) {
        RecomputeHeight = true;
	for (SDNode::use_iterator I = Vi->use_begin(), E = Vi->use_end();
	      I != E; ++I) {
          SDNode *Vk = *I;
          if (I.getUse().getValueType() == MVT::Other)
            DEBUG(errs() << "Vk:Blue edged node\n");
          else if (I.getUse().getValueType() == MVT::Flag)
            DEBUG(errs() << "Vk:Red edged node\n");
          else if (Vk != Vj)
            Vk->setSequencingEdge(Vj);
        }
      }
      // Add Vj to lineage
      l->AddNodeToLineage(Vj);
      // if Vj is already in lineage, then break the lineage
      if (Vj->getIsInLineage() == true)
        break;
      Vj->setIsInLineage(true);
      Vi = Vj;
      // initialize Vj for next iteration
      // Vj = CurDAG->LowestDescendant(Vi);
    }
    // print the lineage
    std::cout << "Lineage:\n";
    l->PrintLineage();
  ...
  DEBUG(errs() << "===== MRIS Lineage Formation end.\n");
}
*/
  // mycode ends here
