...
private:
  ...
  // mycode here
  /// IsInLineage - Boolean value that denotes whether the node is in any
  /// lineage
  bool IsInLineage;

  /// Height - Height of the node in the ddg
  int Height;

  /// NumDescendants - The number of descendants for this node. This includes
  /// descendants as specified by the DFS height ordering, where a node's child
  /// is reached by its corresponding FrameIndex also.
  int NumDescendants;

  /// VisitedHeight - Boolean value that denotes whether the node has been
  /// visited or not
  bool VisitedHeight;

  /// SequencingEdge - The sequencing edge to a SDNode as described in Minimum
  /// Register Instruction Sequencing.
  SDNode *SequencingEdge;

  /// IsScheduled - Indicates whether the SUnit of this SDNode is scheduled
  /// or not. SUnit also maintains isScheduled, but IsScheduled is being used
  /// here for simplicity and keep track of SDNodes that are scheduled.
  bool IsScheduled;

  /// Descendants - The set of descendants of the node. The descendants are
  /// ordered by height for easy retrieval.
  std::list<SDNode*> Descendants;
  // mycode ends here
...
public:
  ...
  // mycode here
  /// getNodeType - Return the NodeType of the SDNode.
  ...
  // mycode here
  /// getIsInLineage - Return the boolean value of whether the node is in any
  /// lineage.
  bool getIsInLineage() const { return IsInLineage; }

  /// setIsInLineage - Set IsInLineage to true or false.
  void setIsInLineage(bool value) { IsInLineage = value; }

  /// getHeight - Return the height of the node in the ddg.
  int getHeight() const { return Height; }

  /// setHeight - Set the height of the node in the ddg.
  void setHeight(int value) { Height = value; }

  /// getNumDescendants - Return the number of descendants for this node.
  int getNumDescendants() const { return NumDescendants; }

  /// setNumDescendants - Set the number of descendants for this node.
  void setNumDescendants(int value) { NumDescendants = value; }

  /// getVisitedHeight - Return the boolean value of whether the node has been
  /// visited or not.
  bool getVisitedHeight() const { return VisitedHeight; }

  /// setVisitedHeight - Set VisitedHeight to true or false.
  void setVisitedHeight(bool value) { VisitedHeight = value; }

  /// getSequencingEdge - Return the SequencingEdge SDNode.
  SDNode *getSequencingEdge() { return SequencingEdge; }

  /// setSequencingEdge - Set the SequencingEdge SDNode.
  void setSequencingEdge(SDNode *N) { SequencingEdge = N; }

  /// getIsScheduled - Return the boolean value of whether the node has been
  /// scheduled or not.
  bool getIsScheduled() const { return IsScheduled; }

  /// setIsScheduled - Set IsScheduled to true or false.
  void setIsScheduled(bool value) { IsScheduled = value; }

  /// isLeafNode - check if the value passed is a leafnode as per ISD.
  static bool isLeafNode(int value) {
    switch (value) {
      case ISD::BasicBlock:
      case ISD::VALUETYPE:
      case ISD::CONDCODE:
      ...
      case ISD::BlockAddress: return true; break;
      default: return false;
    }
  }

  /// Descendant iterators
  typedef std::list<SDNode*>::iterator descendant_iterator;
  descendant_iterator descendant_begin() { return Descendants.begin(); }
  descendant_iterator descendant_end() { return Descendants.end();}

  /// getDescendant - Returns the descendant with least height and
  /// with least preference to load/store nodes. This is because
  /// load/store nodes halts building lineages abruptly.
  SDNode* getDescendant() {
    if (Descendants.size() == 0)
      return NULL;
    SDNode *LS = NULL;
    for (std::list<SDNode*>::iterator I = Descendants.begin(),
          E = Descendants.end(); I != E; ++I) {
      SDNode *temp = *I;
  ...
  /// clearDescendant - Clears the Descendants' list.
  void clearDescendants() { Descendants.clear(); }

  /// setDescendant - Sets the descendant of the SUnit in order by height
  void setDescendant(SDNode *N) {
    bool inserted = false;
    for (std::list<SDNode*>::iterator I = Descendants.begin(),
          E = Descendants.end(); I != E; ++I) {
      SDNode *temp = *I;
      if (N->getHeight() < temp->getHeight()) {
        Descendants.insert(I, N);
        inserted = true;
      }
    }
    if (!inserted)
