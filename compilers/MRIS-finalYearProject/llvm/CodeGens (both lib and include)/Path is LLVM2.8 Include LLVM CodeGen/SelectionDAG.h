...
public:
  ...
  /// in any lineage.
  //SDNode *getHighestNodeNotInAnyLineage();
  ...
  unsigned MarkAllNodesAsNotInAnyLineage();
  ...
  void PrintHeightOfNodes();

  /// PrintSequenceNodes - Print the nodes connected by sequencing edges.
  void PrintSequenceNodes();

  /// setVisitedHeightToFalse - Set VisitedHeight of all nodes to false.
  void setVisitedHeightToFalse();
	
  /// setIsScheduledToFalse - Set IsScheduled of all nodes to false.
  void setIsScheduledToFalse();

  /// CheckForPredsSched - Return true if all the predecessors of SDNode
  /// are scheduled.
  bool CheckForPredsSched(SDNode *N);

  /// ComputeHeightOfEveryNode - Compute the height of every node in the ddg.
  void ComputeHeightOfEveryNode();

  /// ComputeHeightOfNode - Compute the height of node N recursively as
  /// height (u) = 1, if 'u' has no descendants
  ///            = 1 + max (descendants height) of 'u', otherwise.
  int ComputeHeightOfNode(SDNode *N);
